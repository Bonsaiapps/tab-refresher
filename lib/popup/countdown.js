...
function (module) {
  ...
  */
	formatList = function (ts) {
		...
		if (ts.days) {
			list.push(plurality(ts.days, 'd', 'd'));
		}
		if (ts.hours) {
			list.push(plurality(ts.hours, 'h', 'h'));
		}
		if (ts.minutes) {
			list.push(plurality(ts.minutes, 'm', 'm'));
		}
		if (ts.seconds) {
			list.push(plurality(ts.seconds, 's', 's'));
		}
		if (ts.milliseconds) {
  ...
  */
	function countdown(start, end, units) {
		...
		var fn = function fn() {
			callback(populate(new Timespan(), /** @type{Date} */start || new Date(), /** @type{Date} */end || new Date(), units));
		};
