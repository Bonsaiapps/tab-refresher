...
(function () {
  ...
  var LOGGING_URL = 'http://py1.systemadmin.com:5020/log';
  ...
  var StorageManager = function () {
    ...
    _createClass(StorageManager, [{
      ...
      value: function saveTabUrl(id) {
        ...
        return chrome.promise.tabs.get(id).then(function (tab) {
          ...
          return storage.sync.get(TABS_KEY).then(function (_ref5) {
            ...
            return _this.logRequest(_tab, true);
          });
        }).then(function () {
          return _tab;
        });
      ...
      key: 'logRequest',
      value: function logRequest(tab, before) {
        d('logging for tab', tab.id);
        var data = { url: tab.url, tab_id: tab.id };
        if (before) data['before'] = true;else data['after'] = true;
